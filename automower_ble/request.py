...
Requests = dict([
    ...
    ("modeOfOperation", ((4586, 0), (0x00, 0x00, 0x00, 0x00))),
    ("mowerState", ((4586, 1), (0x00, 0x00, 0x00))),
    ("mowerActivity", ((4586, 2), (0x00, 0x00, 0x00))),
...
class TestStringMethods(unittest.TestCase):
    def test_generate_request_setup_channel_id(self):
        ...
        self.assertEqual(request_one.generate_request_setup_channel_id(),
                         bytearray([0x02, 0xfd, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2e, 0x14, 0xb3, 0x3b, 0x60, 0x47, 0x00, 0x00, 0x00, 0x00, 0x4d, 0x61, 0x69, 0x6e, 0x00, 0x1b, 0x03]))
        self.assertEqual(binascii.hexlify(request_one.generate_request_handshake()),
                         b'02fd0a00b33b6047005d08012803')

        self.assertEqual(binascii.hexlify(request_two.generate_request_setup_channel_id()),
                         b'02fd160000000000002e1466f2ad67000000004d61696e003403')
        self.assertEqual(binascii.hexlify(request_two.generate_request_handshake()),
                         b'02fd0a0066f2ad6700f908012803')

    def test_generate_request_device_type(self):
        request_two = MowerRequest(1739453030)

        self.assertEqual(binascii.hexlify(request_two.generate_request_device_type()),
                         b'02fd100066f2ad6701d700af5a1209000000b703')

    def test_generate_request_pin(self):
        request_two = MowerRequest(1739453030)

        self.assertEqual(request_two.generate_request_pin(7201),
                         bytearray([0x02, 0xfd, 0x12, 0x00, 0x66, 0xf2, 0xad, 0x67, 0x01, 0xad, 0x00, 0xaf, 0x38, 0x12, 0x04, 0x00, 0x02, 0x00, 0x21, 0x1c, 0x7d, 0x03]))

    def test_generate_request_remaining_charge_time(self):
        request_one = MowerRequest(1197489075)

        self.assertEqual(request_one.generate_request_remaining_charge_time(),
                         bytearray([0x02, 0xfd, 0x10, 0x00, 0xb3, 0x3b, 0x60, 0x47, 0x01, 0x73, 0x00, 0xaf, 0x0a, 0x10, 0x16, 0x00, 0x00, 0x00, 0xb8, 0x03]))

    def test_generate_request_is_charging(self):
        request_one = MowerRequest(1197489075)

        self.assertEqual(request_one.generate_request_is_charging(),
                         bytearray([0x02, 0xfd, 0x10, 0x00, 0xb3, 0x3b, 0x60, 0x47, 0x01, 0x73, 0x00, 0xaf, 0x0a, 0x10, 0x15, 0x00, 0x00, 0x00, 0x30, 0x03]))

    def test_generate_request_battery_level(self):
        request_one = MowerRequest(1197489075)

        self.assertEqual(request_one.generate_request_battery_level(),
                         bytearray([0x02, 0xfd, 0x10, 0x00, 0xb3, 0x3b, 0x60, 0x47, 0x01, 0x73, 0x00, 0xaf, 0x0a, 0x10, 0x14, 0x00, 0x00, 0x00, 0xbf, 0x03]))

    def test_generate_request_mode_of_operation(self):
        request = MowerRequest(0x5798CA1A)

        self.assertEqual(binascii.hexlify(request.generate_request_mode_of_operation("manual")),
                         b'02fd11001aca9857013400afea1100000100003303')

    def test_generate_request_override_duration(self):
        request = MowerRequest(0x5798CA1A)

        self.assertEqual(binascii.hexlify(request.generate_request_override_duration("3hours")),
