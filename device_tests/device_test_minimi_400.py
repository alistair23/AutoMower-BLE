...
async def main(mower: Mower, must_start: bool = False, must_stop: bool = False):
    ...
    try:
        ...
        logger.debug(
            f'Next task starts at {task_response.next_start_time.strftime("%H:%M:%S")}'
        )
        logger.debug(
            f"Next task has a duration of {task_response.duration_in_seconds} seconds"
        )
        ...
        if must_start:
            ...
            await mower.set_mower_override_duration_in_seconds(
                30 * 60
...
if __name__ == "__main__":
    ...
    device_group.add_argument("--start", action="store_true", help="start mowing")
    device_group.add_argument("--stop", action="store_true", help="stop mowing")
    ...
    if args.start and args.stop:
        logger.error(
            "There is an issue. You cannot start and stop both at the same time"
        )
